...
base_url = r'http://cs.fangjia.com/ershoufang/'


test_search_dict = {'昌平': {'霍营': {'13号线': 'http://cs.fangjia.com/ershoufang/--r-%E6%98%8C%E5%B9%B3|w-13%E5%8F%B7%E7%BA%BF|b-%E9%9C%8D%E8%90%A5'}}}
...
def get_info_pn_list(search_list):
    ...
    for i in range(len(search_list)):
        ...
        # 获取最大页数
        pn_num = 2
        #soup.select('span[class="mr5"]')[0].get_text()
        rule = re.compile(r'\d+')
        max_pn =pn_num
        # 组装url
...
def save_excel(fin_info_list, file_name):
    ...
    book = xlsxwriter.Workbook(r'C:\Users\Esri\Desktop\%s.xlsx' % file_name)  # 默认存储在桌面上
...
if __name__ == '__main__':
    ...
    p = Pool(8)  # 设置进程池
    assignment_list = assignment_search_list(fin_info_pn_list, 6)  # 分配任务，用于多进程
    result = []  # 多进程结果列表
